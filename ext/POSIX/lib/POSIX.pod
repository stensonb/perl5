...
characters of the C standards, except under Unicode UTF-8 locales.

So

Use C<undef> as the first parameter to this function to get the effect
of passing NULL as the first parameter to the C function.  Note that the
returned value is different here than when the first parameter is
defined.  It is 0 if the locale is stateless; and non-zero otherwise.

C<mbrlen> is transparently used instead on threaded perls, if available
on the platform, so as to avoid thread race conditions.
