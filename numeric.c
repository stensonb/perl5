...
returns C<UV_MAX>, sets C<PERL_SCAN_GREATER_THAN_UV_MAX> in C<*flags>,
and writes the value to C<*result> (or the value is discarded if C<result>
is C<NULL>).
...
Perl_grok_hex(pTHX_ const char *start, STRLEN *len_p, I32 *flags, NV *result)
{
    ...
    const bool allow_underscores = cBOOL(input_flags & PERL_SCAN_ALLOW_UNDERSCORES);
        ...
        if (isXDIGIT(*s)) {
          ...
          redo:
            if (!overflowed) {
                ...
                if (! (input_flags & PERL_SCAN_GREATER_THAN_UV_MAX)) {
...
UV
Perl_grok_bin_oct(pTHX_ const char *start,
                        STRLEN *len_p,
                        I32 *flags,
                        NV *result,
                        const unsigned shift) /* 1 for binary; 3 for octal */
{
    ...
    const PERL_UINT_FAST8_T count = base - 1;
    I32 input_flags = *flags;
    const bool allow_underscores
                            = cBOOL(input_flags & PERL_SCAN_ALLOW_UNDERSCORES);
    bool overflowed = FALSE;

    PERL_ARGS_ASSERT_GROK_BIN_OCT;

    *flags = 0;

    if (base == 2 && !(input_flags & PERL_SCAN_DISALLOW_PREFIX)) {
    ...
    for (; len-- && *s; s++) {
            ...
            &&  ckWARN(WARN_DIGIT))
        {
            if (base == 2) {
                ...
                /* Allow octal base to work the DWIM way (that is, stop
                 * scanning as soon as alien characters are seen, complain only
                 * if someone seems to want to use the digits eight and nine.
...
Perl_scan_bin(pTHX_ const char *start, STRLEN len, STRLEN *retlen)
{
    ...
    const UV ruv = grok_bin_oct (start, &len, &flags, &rnv, 1);
...
Perl_scan_oct(pTHX_ const char *start, STRLEN len, STRLEN *retlen)
{
    NV rnv;
    I32 flags = *retlen ? PERL_SCAN_ALLOW_UNDERSCORES : 0;
    const UV ruv = grok_bin_oct (start, &len, &flags, &rnv, 2);
